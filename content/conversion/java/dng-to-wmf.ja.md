...
breadcrumb: JavaでDNGからWMFへ

############################# Head ############################
head_title: "Java で DNG を WMF に変換します"
head_description: "Java での DNG から WMF への変換を、数行のコードで行います。 Java の GroupDocs ドキュメント変換 API を使用して、160 を超えるファイル形式を変換します"

############################# Header ############################
title: "Java で DNG を WMF に変換します"
description: "数行の Java コードによる DNG から WMF への変換"
...
overview:
    enable: true
    content: |
        Java で DNG ファイルを WMF に変換します。 Windows、Linux、macOS など、任意のプラットフォームで数行の Java コードを実行するだけです。
        DNG を WMF に無料で変換してみて、変換結果の品質を評価できます。シンプルなファイル変換スクリプトに加えて、DNG ソース ファイルをロードし、WMF 出力を保存するためのより洗練されたオプションを試すことができます。 
        ...
        * 変換された WMF に透かしを追加します.
        ...
        変換が完了したら、WMF ファイルをローカル ファイル パス、または FTP、Amazon S3、Google ドライブ、Dropbox などのサード パーティのストレージに保存できます。注意してください - DNG を変換するにはWMF に、MS Office、Open Office、Adobe Acrobat Reader などの追加のソフトウェアをインストールする必要はありません。


############################# Steps ############################
steps:
    enable: true
    title_left: "Java で DNG を WMF に変換する手順"
    content_left: |
        [GroupDocs.Conversion for Java](https://products.groupdocs.com/conversion/java/) を使用すると、開発者は数行のコードで DNG ファイルを WMF に簡単に変換できます。
...
demos:
    ...
    content: |
       [GroupDocs.Conversion アプリ](https://products.groupdocs.app/conversion/family) ウェブサイトにアクセスして、今すぐ DNG から WMF への変換を試してください。無料デモには次の利点があります
