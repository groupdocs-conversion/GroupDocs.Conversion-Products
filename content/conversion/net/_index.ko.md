...
head_description: "C# .NET 문서 변환 API. PDF Word DOC DOCX, Excel 스프레드시트 PPT PPTX, HTML, PSD, MPT MPP, 이메일 MSG EMLX, AutoCAD 및 이미지 파일 형식 변환."

title: ".80개 이상의 파일 형식을 변환하는 NET API"
description: "외부 소프트웨어를 설치하지 않고 문서 및 이미지 변환 기능을 .NET 응용 프로그램에 통합하는 간단한 API."
button:
    enable: true
...
overview:
    enable: true
    content: |
      .NET용 GroupDocs.Conversion은 개발자가 C#, ASP.NET 및 기타 .NET 관련 기술에서 강력한 문서 변환 응용 프로그램을 빌드할 수 있도록 하는 간단한 API 집합을 제공합니다. .NET API용 GroupDocs.Conversion은 최종 사용자에게 빠르고 효율적이며 안정적인 파일 변환 솔루션을 제공합니다. PDF, HTML, 이메일, Microsoft Word 문서, Excel 스프레드시트, PowerPoint 프레젠테이션, 프로젝트, Photoshop, CorelDraw, AutoCAD, 다이어그램, 래스터 이미지 파일 형식 등을 포함한 모든 인기 있는 비즈니스 문서 형식 간의 정확한 변환을 지원합니다. 문서 변환기 라이브러리는 소스 문서 형식을 자동으로 감지하고 전체 문서 또는 특정 페이지를 원하는 출력 형식으로 변환하는 모든 제어 기능을 제공합니다. 누락된 글꼴을 선호하는 글꼴로 교체하고 문서 페이지에 텍스트 또는 이미지 워터마크를 추가하는 것이 더 쉽습니다.  
        
      .NET용 GroupDocs.Conversion은 .NET 플랫폼을 대상으로 하는 모든 개발 환경에서 응용 프로그램을 개발하는 데 사용할 수 있습니다. 모든 .NET 기반 언어와 호환되며 Mono 또는 .NET 프레임워크(.NET Core 포함)를 설치할 수 있는 인기 있는 운영 체제(Windows, Linux, MacOS)를 지원합니다.
    tabs:
      enable: true
      
      tab_one:
        description: |
          다음은 .NET용 GroupDocs.Conversion의 개요입니다.
      ...
      tab_two:
        description: |
          .NET용 GroupDocs.Conversion은 널리 사용되는 모든 [문서 파일 형식](https://docs.groupdocs.com/conversion/net/supported-document-formats/) 간의 변환을 지원합니다.
      ...
      tab_three:
        description: |
          .NET용 GroupDocs.Conversion은 다음 운영 체제, 프레임워크 및 패키지 관리자를 지원합니다.
...
features:
    enable: true
    title: ".NET 기능을 위한 GroupDocs.Conversion"
    ...
    feature:
      ...
      - icon: "fas fa-bolt"
        content: "모든 인기 있는 래스터 이미지 형식으로/에서 변환 및 이미지 DPI, 높이 및 너비 할당"
      ...
      - icon: "fas fa-cloud"
        content: "변환된 문서의 워터마크를 텍스트 뒤에 표시할 배경으로 구성 및 배치"
      ...
      - icon: "fas fa-border-all"
        content: "출력을 경로 또는 IO 스트림으로 반환하는 고급 변환 방법"

      - icon: "fas fa-wrench"
        content: "그리드 라인이 있는 스프레드시트 변환 및 변환하는 동안 슬라이드에서 주석 제거"
      ...
      - icon: "fas fa-envelope"
        content: "문서의 총 페이지 수를 계산하고 변환하는 동안 암호를 보호되지 않은 문서로 설정"
      ...
      - icon: "fas fa-file-excel"
        content: "셰이딩 옵션이 있는 DOCX에서 Tiff G3로 변환"
      ...
      - icon: "fab fa-uncharted"
        content: "XML 문서를 변환 없이 모든 형식으로 변환"
    ...
    more_feature:
      - title: "문서 형식을 쉽게 변환"
        content: |
          .NET용 GroupDocs.Conversion을 사용하여 문서 파일 형식을 변환하는 것은 매우 쉽습니다. 다음 예에서는 C#을 사용하여 PDF 파일을 DOC 파일로 변환하는 방법을 보여줍니다.  
            
          * 파일 경로를 입력으로 가져옵니다.
          * 파일 저장 경로 가져오기
          * 입력 문서 파일 변환
          * 출력을 경로 또는 스트림으로 반환(요구 사항에 따라)  
            
            
          ```cs
          // 변환 구성 설정 및 ConversionHandler 초기화
          ConversionConfig config = new ConversionConfig();
          config.StoragePath = @"D:\storage\"; 
          // ConversionHandler 초기화
      ...
      - title: "이미지 형식으로 변환"
        content: ".NET API용 GroupDocs.Conversion은 다양한 이미지 형식 간의 문서 변환을 지원합니다. 다른 형식에서 이미지로 변환하는 동안 이미지의 다양한 속성을 조정할 수 있습니다. 우리의 변환 API를 사용하면 밝기, 대비, 감마 속성을 조정할 수 있습니다. 이미지를 뒤집을 수도 있습니다. 그리고 JPEG 형식으로 변환하는 경우 파일의 색상 모드와 압축 모드를 설정할 수 있습니다.."

      - title: "다양한 PDF 형식 지원"
        content: |
          .NET API용 GroupDocs.Conversion은 다음 PDF 유형/형식으로 문서 변환을 지원합니다.  
...
solutions:
    enable: true
    title: "GroupDocs.Conversion은 다른 인기 있는 개발 환경을 위한 문서 보기 API를 제공합니다."
