...
############################# Head #############################
head_title: ".NET용 DOTX에서 DOCX 변환기 API - C# VB.NET으로 변환"
head_description: "DOTX을(를) DOCX로 변환하는 .NET 파일 변환기 API. C#, ASP.NET, VB.NET 및 .NET Core 애플리케이션에서 사용 가능."

############################# Header ############################
title: "C# .NET 응용 프로그램용 DOTX에서 DOCX 파일 변환기" 
description: "DOTX에서 DOCX로의 원활한 변환을 위한 .NET 문서 변환 API. 변환 과정에서 정확한 서식 및 레이아웃 보존을 보장하면서 코딩 작업을 최소화합니다." 
subtitle: "문서 변환 솔루션" 
...
steps:
    enable: true
    title: "C#에서 DOTX 파일을 DOCX 파일로 변환하는 단계" 
    content: |
      <a href='https://products.groupdocs.com/conversion/net/'>GroupDocs.Conversion</a>를 사용하면 DOTX을(를) DOCX로 변환하는 과정이 몇 가지 간단한 단계로 가능합니다.
    ...
    code:
      ...
      content: |
        ```csharp {style=abap}
        FluentConverter
            .Load("input.dotx")             // DOTX 파일을 입력 설정
            .ConvertTo("converted.docx")     // 변환된 파일의 출력 경로 지정
            .Convert();                     // DOTX 파일을 DOCX 파일로 변환        
        ```            
