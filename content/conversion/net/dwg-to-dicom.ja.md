...
############################# Head #############################
head_title: ".NET 向け DWG から DICOM コンバーター API - C# VB.NET で変換"
head_description: "DWG を DICOM に変換する .NET ファイルコンバーター API。C#、ASP.NET、VB.NET、.NET Core アプリケーションで利用可能。"

############################# Header ############################
title: "C# .NET アプリケーション向け DWG から DICOM ファイルコンバーター" 
description: "DWG から DICOM へのシームレスな変換を実現する .NET ドキュメントコンバーター API。C#、ASP.NET、VB.NET、.NET Core アプリケーション内で、変換プロセス中の精度の高いフォーマットとレイアウトの保存を確保し、コーディングの手間を最小限に抑えます。" 
...
steps:
    enable: true
    title: "C# で DWG ファイルを DICOM ファイルに変換する手順" 
    content: |
      <a href='https://products.groupdocs.com/conversion/net/'>GroupDocs.Conversion</a> を使用して、DWG を DICOM に変換する手順はいくつかの簡単なステップだけです。
    ...
    code:
      ...
      content: |
        ...
        FluentConverter
            ...
            .Convert();                     // DWG ファイルを DICOM ファイルに変換        
        ```            
