...
############################# Head #############################
head_title: ".NET 向け XLT から XML コンバーター API - C# VB.NET で変換"
head_description: "XLT を XML に変換する .NET ファイルコンバーター API。C#、ASP.NET、VB.NET、.NET Core アプリケーションで利用可能。"

############################# Header ############################
title: "C# .NET アプリケーション向け XLT から XML ファイルコンバーター" 
description: "XLT から XML へのシームレスな変換を実現する .NET ドキュメントコンバーター API。C#、ASP.NET、VB.NET、.NET Core アプリケーション内で、変換プロセス中の精度の高いフォーマットとレイアウトの保存を確保し、コーディングの手間を最小限に抑えます。" 
...
steps:
    enable: true
    title: "C# で XLT ファイルを XML ファイルに変換する手順" 
    content: |
      <a href='https://products.groupdocs.com/conversion/net/'>GroupDocs.Conversion</a> を使用して、XLT を XML に変換する手順はいくつかの簡単なステップだけです。
    ...
    code:
      ...
      content: |
        ```csharp {style=abap}
        FluentConverter
            .Load("input.xlt")             // XLT ファイルを入力設定
            .ConvertTo("converted.xml")     // 変換されたファイルの出力パスを指定
            .Convert();                     // XLT ファイルを XML ファイルに変換        
        ```            
